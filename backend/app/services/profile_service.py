...
class ProfileService:
    ...
    def get_student_profile_by_id(self, current_user: User, student_id: int) -> StudentProfileResponse:
        # Allow admins, employers, or the student themself to view a student profile
        from app.models.enums import UserRole

        student = self.profile_repository.get_student_by_id(student_id)
        if not student:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="Профиль студента не найден",
        ...
        # If requester is a student, allow only viewing own profile
        if current_user.role == UserRole.STUDENT:
            if not student or current_user.id != student.id:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="Доступ запрещен",
                )

        # Employers and admins can view student profiles
        # Build response using available user fields if present
        user = None
